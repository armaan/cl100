...
# Replace run name.
for i in course.xml chapter/*.xml policies/_base/policy.json static/markdown/*.md; do
    sed -i "s/{{ RUN }}/$RUN/" $i
done
...
for i in course.xml chapter/*.xml policies/_base/policy.json policies/$RUN static/markdown; do
    git add $i
done
